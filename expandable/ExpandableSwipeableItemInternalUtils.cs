...
namespace Com.H6ah4i.Android.Widget.Advrecyclerview.Expandable
{
    public class ExpandableSwipeableItemInternalUtils
    {
        ...
        public static SwipeResultAction InvokeOnSwipeItem(
            IBaseExpandableSwipeableItemAdapter adapter,
        RecyclerView.ViewHolder holder,
        int groupPosition, int childPosition, int result)
        {
            if (childPosition == RecyclerView.NoPosition)
            {
                return ((IExpandableSwipeableItemAdapter)adapter).OnSwipeGroupItem(holder, groupPosition, result);
            }
            else
            {
                return ((IExpandableSwipeableItemAdapter)adapter).OnSwipeChildItem(holder, groupPosition, childPosition, result);
            }
        }
    }
}